    ...
    <div class="row">
        <div class="col-md-8 col-md-offset-2">
            <div class="panel panel-default">
                <div class="panel-heading">Edit this Contact</div>
                <div class="panel-body">
                	@if(isset($result))
	                <p>Contact Updated successfully!</p>
	                @else
                    <form class="form-horizontal" role="form" method="POST" action="{{ url('/contacts/'.$contact->id) }}">
                    {{ method_field('PUT') }}
                        {{ csrf_field() }}
                        <div class="form-group{{ $errors->has('contactname') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="contactname" type="text" class="form-control" name="contactname" value="{{ $contact->contactname }}" required autofocus>
                        ...
                        <div class="form-group{{ $errors->has('phonenumber') ? ' has-error' : '' }}">
                            ...
                            <div class="col-md-6">
                                <input id="phonenumber" type="text" class="form-control" name="phonenumber" value="{{ $contact->phonenumber }}" required autofocus>
                            ...
                            <div class="col-md-6">
                                <select id="phonetype" class="form-control" name="phonetype_id" autofocus>
                                	@foreach ($types as $id=>$phonetype)
                                		<option value="{{$id}}" {{ ($id == $contact->phonetype_id) ? 'selected' : '' }}>{{$phonetype}}</option>
                                	@endforeach
                                </select>
                            </div>
                        </div>
